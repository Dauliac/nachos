...
//----------------------------------------------------------------------
void
Interrupt::Schedule (VoidFunctionPtr handler, void *arg, long long fromNow,
		     IntType type)
{
    long long when = stats->totalTicks + fromNow;
    PendingInterrupt *toOccur =
	new PendingInterrupt (handler, arg, when, type);
...
bool Interrupt::CheckIfDue (bool advanceClock)
{
	...
	DumpState ();
    PendingInterrupt *
	toOccur = (PendingInterrupt *) pending->SortedRemove (&when);
